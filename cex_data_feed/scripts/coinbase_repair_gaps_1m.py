#!/usr/bin/env python3
"""
Repair gaps in the 1m BTC-USD Coinbase SQLite database.

Scans the DB for missing 1-minute candles, fetches them from Coinbase Exchange,
...
running this on every cron tick — use coinbase_accumulate_1m for that.

Usage:
  python -m cex_data_feed.scripts.coinbase_repair_gaps_1m \\
...
from cex_data_feed.pipeline_1m_coinbase.sqlite_db import (
    ensure_table, insert_candles, coverage_stats, find_first_gap,
)
...
def run_once(
    ...
    if has_trailing_gap:
        ...
        df = fetch_closed_1m_since(start_ts=trailing_start, product_id=product_id)
        if not df.empty:
            if dry_run:
    ...
    if gap_ts is not None:
        ...
        df = fetch_closed_1m_since(start_ts=gap_ts, product_id=product_id)
        if not df.empty:
            if dry_run:
                print(f"[{now_str}] [DRY-RUN] Would insert {len(df)} candles")
            else:
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Repair gaps in the 1m BTC-USD Coinbase SQLite database",
