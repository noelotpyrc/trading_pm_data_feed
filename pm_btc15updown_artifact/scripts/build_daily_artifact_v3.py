#!/usr/bin/env python3
"""
Build daily volatility signal artifact (V3: per-TTL MAR, 15 models, 10 predictors).

Runs on VPS once per day before the trading node starts. Produces:
  - model.json   (linear regression coefficients per MAR horizon 1–15)
  - z_pool.npy   (empirical z-score distribution)
  - metadata.json (build info, coverage stats)

Usage:
  python -m pm_btc15updown_artifact.scripts.build_daily_artifact_v3 \
    --db data/btcusdt_perp_1m.sqlite \
    --out-dir data/artifacts_v3 \
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Build daily volatility signal artifact V3 (per-TTL MAR) from 1m OHLCV SQLite",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    p.add_argument("--db", type=Path, required=True,
                   help="Path to 1m OHLCV SQLite database")
    p.add_argument("--out-dir", type=Path, default=Path("data/artifacts_v3"),
