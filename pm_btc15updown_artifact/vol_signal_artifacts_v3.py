...
class VolSignalBuildConfigV3:
    ...
    vol_windows: tuple[int, ...] = (5, 10, 15, 30, 45, 60, 1440)
    mar_horizons: tuple[int, ...] = tuple(range(1, 16))
...
class DailySignalArtifactV3:
    ...
    @classmethod
    def load(cls, artifact_dir: Path) -> DailySignalArtifactV3:
...
def prepare_feature_frame(df: pd.DataFrame, config: VolSignalBuildConfigV3 | None = None) -> pd.DataFrame:
    config = config or VolSignalBuildConfigV3()
    out = validate_ohlcv_frame(df)

    # Parkinson volatility windows
    ...
    # Realized MAR features (backward-looking)
    log_ret_abs = np.log(out["close"] / out["close"].shift(1)).abs()
    out["realized_mar_1"] = log_ret_abs
    for w in (2, 3, 4):
        out[f"realized_mar_{w}"] = log_ret_abs.rolling(w).mean()

    # Forward log returns
    for n in config.forward_return_horizons:
        out[f"target_fwd_ret_{n}"] = np.log(out["close"].shift(-n) / out["close"])

    # Forward MAR targets (all 15 horizons)
...
# ---------------------------------------------------------------------------
# Signal calculation (V3: per-TTL MAR, no blending)
# ---------------------------------------------------------------------------

def per_ttl_mar(df: pd.DataFrame, config: VolSignalBuildConfigV3 | None = None) -> pd.Series:
    """Select pred_mar_{ttl} for each row based on its TTL value."""
    config = config or VolSignalBuildConfigV3()
    mar = pd.Series(np.nan, index=df.index, dtype=float)
    for ttl in range(1, config.group_minutes + 1):
        mask = df["ttl"] == ttl
        col = f"pred_mar_{ttl}"
        if col in df.columns:
            mar.loc[mask] = df.loc[mask, col].to_numpy()
    return mar


def sigma_w_from_mar(mar_per_ttl: pd.Series, ttl: pd.Series) -> pd.Series:
    mar_clean = pd.to_numeric(mar_per_ttl, errors="coerce").clip(lower=0.0)
    sigma_1m = math.sqrt(math.pi / 2.0) * mar_clean
    return sigma_1m * np.sqrt(pd.to_numeric(ttl, errors="coerce"))


def attach_signal_columns(df: pd.DataFrame, config: VolSignalBuildConfigV3 | None = None) -> pd.DataFrame:
    config = config or VolSignalBuildConfigV3()
    out = df.copy()
    ...
    out["mar_per_ttl"] = per_ttl_mar(out, config)
    out["sigma_W_ttl"] = sigma_w_from_mar(out["mar_per_ttl"], out["ttl"])
    return out
...
def score_date_range_for_history(
    ...
    config: VolSignalBuildConfigV3 | None = None,
) -> pd.DataFrame:
    config = config or VolSignalBuildConfigV3()
