...
def _extract_token(snap: dict, idx: int) -> dict:
    ...
    return {
        ...
        # older snapshot dicts that pre-date the fetch_prices change.
...
def _stale_metrics(polls: list[dict]) -> dict:
    ...
    i.e. the server reports the book has not been updated. This is the
    fingerprint we saw on 2026-05-06 when both directional/contrarian
    fired against frozen orderbooks.
    """
    n = len(polls)
    n_unchanged = 0
    for i in range(1, n):
...
def _is_stale(metrics: dict) -> bool:
    """True if all polls saw the same book (book_hash distinct count == 1)
    and we had at least 2 polls to compare. Pre-feature snapshots without
    book_hash will yield distinct=0 → False (treated as 'unknown', not stale).
    """
...
def _format_summary(job: TriggerJob, settlement: float) -> str:
    ...
    sm3 = _stale_metrics(job.polls)
    stale_tag = ""
    if _is_stale(sm3):
        stale_tag = f" [⚠ STALE {sm3['n_unchanged']}/{sm3['n_polls']}]"

    lines = [
        f"{emoji} **Directional {job.target_outcome}**{stale_tag} "
...
def _summary_payload(job: TriggerJob) -> dict:
    ...
    sm3 = _stale_metrics(job.polls)
    sm10 = _stale_metrics(job.extended_polls)
    return {
        ...
        # Staleness diagnostics — see _stale_metrics + _is_stale
        "stale": _is_stale(sm3),
        "stale_3s_n_unchanged": sm3["n_unchanged"],
        "stale_3s_unique_hash": sm3["unique_book_hash"],
        "stale_3s_book_ts_span_ms": sm3["book_ts_span_ms"],
        "stale_10s_n_unchanged": sm10["n_unchanged"],
        "stale_10s_unique_hash": sm10["unique_book_hash"],
        "stale_10s_book_ts_span_ms": sm10["book_ts_span_ms"],
